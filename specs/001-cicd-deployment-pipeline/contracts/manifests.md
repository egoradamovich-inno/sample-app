...
8.5. `kubectl apply -f k8s/domain-claim.yaml -f k8s/domain-mapping.yaml -n sample-app` (idempotent), then `kubectl wait --for=condition=Ready domainmapping/sample-app.cert.local -n sample-app --timeout=60s`. Discovered via a real deploy, not anticipated at plan time: the Knative Ingress never registers `sample-app.cert.local` as a route at all on its own — its default-generated hosts are only the internal `sample-app.sample-app[.svc[.cluster.local]]` forms (`config-domain`'s `domain-template` default), confirmed by a 404 straight from Kourier even for that exact internal host, while the app itself (direct pod port-forward) and nginx (Host header forwarding, config inspected directly) were both independently verified healthy. Must run after step 8 (the `DomainMapping`'s `spec.ref` needs the Knative Service to already exist) and before step 9 (the reachability check needs the mapping actually routing, not just applied). `ClusterDomainClaim` is required explicitly, not auto-created: `config-network`'s `autocreate-cluster-domain-claims` has no override on this cluster, so Knative's built-in default (`false`) applies. No `spec.tls` on the `DomainMapping`: `config-network`'s `external-domain-tls` is likewise unoverridden, defaulting to `Disabled` — this maps to plain HTTP, matching what nginx already expects when it proxies to Kourier (nginx owns TLS termination itself, per Principle I).
