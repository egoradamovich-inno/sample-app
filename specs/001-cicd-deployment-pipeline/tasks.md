...
- [ ] T013 [US1] Checkpoint: manually run quickstart.md Scenario 1 — push a commit, confirm in the Actions run: lint/SAST/Trivy-report/Trivy-gate results are each visible from the run's own record (SC-004), and a new private image tagged to that commit's SHA appears in GHCR (Independent Test satisfied)
...
- [X] T028 [US2] In the `deploy` job, render and apply the Knative Service: `IMAGE=ghcr.io/${{ github.repository_owner }}/sample-app:${{ github.sha }} envsubst < k8s/knative-service.template.yaml | kn service apply sample-app -n sample-app -f -` (depends on T020, T027 succeeding — Contract 3 step 8)
