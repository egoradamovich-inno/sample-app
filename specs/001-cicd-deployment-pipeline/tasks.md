...
- [X] T020a [US2] **Added during `/speckit-implement`, not in the original task breakdown**: create `k8s/domain-claim.yaml` (`ClusterDomainClaim`, `networking.internal.knative.dev/v1alpha1`, cluster-scoped, name `sample-app.cert.local`, `spec.namespace: sample-app`) and `k8s/domain-mapping.yaml` (`DomainMapping`, `serving.knative.dev/v1beta1`, namespace `sample-app`, name `sample-app.cert.local`, `spec.ref: {name: sample-app, kind: Service, apiVersion: serving.knative.dev/v1}`, no `spec.tls`). Discovered via a real deploy: the Knative Ingress never registers `sample-app.cert.local` as a route at all on its own (only the internal `sample-app.sample-app[.svc[.cluster.local]]` forms) — confirmed with a 404 straight from Kourier for the exact internal host, while the app (direct pod port-forward) and nginx (Host-header forwarding, config inspected directly) were both independently verified healthy. Both API versions and the `autocreate-cluster-domain-claims`/`external-domain-tls` defaults (both unoverridden → `false`/`Disabled`) verified against the live cluster (`kubectl api-resources`, `kubectl explain domainmapping`, `kubectl get cm config-network -n knative-serving`), not assumed from docs (different Knative doc snapshots showed inconsistent API versions for these two resources)
...
- [X] T028a [US2] **Added during `/speckit-implement`, not in the original task breakdown**: in the `deploy` job, add `kubectl apply -f k8s/domain-claim.yaml -f k8s/domain-mapping.yaml -n sample-app` (idempotent) followed by `kubectl wait --for=condition=Ready domainmapping/sample-app.cert.local -n sample-app --timeout=60s` (depends on T020a, T028 — Contract 3 step 8.5, between the Knative Service and the reachability check) — the `DomainMapping`'s `spec.ref` needs the Service to already exist, and the reachability check needs the mapping actually routing, not just accepted by the API server
...
- [X] T030 [US2] Checkpoint: manually run quickstart.md Scenario 2 against a clean `sample-app` namespace (nothing pre-deployed) — confirm namespace/secrets/Postgres/migration Job/Knative Service are all created and the `curl -k` check succeeds with a real HTML body (Independent Test satisfied). **Confirmed** via run `36104278461` (2026-09-25): full pipeline green end to end, including the new domain-mapping step (T028a) and the reachability check; a separate manual `curl -k https://sample-app.cert.local/` returned a real `HTTP 200` with the actual HTML body. ADR-002's platform blocker (sharp/CPU SIMD) resolved and verified; the earlier missing-DomainMapping gap (found via the 3-layer bisection) fixed by T020a/T028a. Namespace was not fully empty at run time (reused from earlier iterative debugging in this same session), so this doesn't independently re-prove first-ever-run idempotency from a truly blank cluster — that's what T034 covers.
...
- [ ] T034 [US3] Checkpoint: execute quickstart.md Scenario 3 against the real cluster — run the pipeline twice back-to-back with no code changes; confirm the second run succeeds, `kubectl get pvc sample-app-postgres-data -n sample-app -o jsonpath='{.metadata.uid}'` is unchanged across both runs, `kubectl get secret sample-app-db-credentials -n sample-app -o jsonpath='{.data.POSTGRES_PASSWORD}'` is byte-identical, and application data created between the two runs is still present afterward. **No longer blocked** (T030's ADR-002 blocker resolved) — not yet explicitly re-run as a dedicated back-to-back check; the many iterative fix/redeploy cycles this session are corroborating-but-informal evidence (same PVC/secret objects survived every redeploy without recreation), not a substitute for this specific checkpoint.
- [ ] T035 [US3] Checkpoint: execute quickstart.md's concurrency check — trigger two pushes seconds apart and confirm in the Actions UI that the second `deploy` job queues behind the first (not running in parallel, not cancelled) per the `concurrency` block from T032. Not blocked by ADR-002 or the domain-mapping fix — not yet executed.
...
- [X] T039 Attach real `curl -k https://sample-app.cert.local` output (status + body) as final evidence of completion, per Constitution Principle VI — a passing workflow run alone is not sufficient. **Confirmed**: `curl -k https://sample-app.cert.local/` → `HTTP 200`, real HTML body (`<!doctype html>...<title>frontend</title>...`), run manually immediately after pipeline run `36104278461` completed green.
- [ ] T040 [P] Manually grep the full logs of each checkpoint run (T013, T030, T034) for the literal `GHCR_PULL_PAT` and `SAMPLE_APP_DB_PASSWORD` secret values, confirming GitHub Actions' built-in masking redacts them everywhere they'd otherwise appear (SC-005 — "no secret value is ever visible in the pipeline's visible output"); this was previously undocumented as a task despite being called out in quickstart.md's Success Criteria mapping. Not yet executed.
